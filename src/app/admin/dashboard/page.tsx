...
export default function AdminDashboard() {
  ...
  const orderStatusCounts = filteredOrders.reduce(
    (acc, order) => {
      ...
      if (order.status === "dispatch") acc.dispatch++;
      if (order.status === "success") acc.success++;
      return acc;
    },
    { pending: 0, dispatch: 0, success: 0 }
  );
  ...
  const barChartData = {
    ...
    datasets: [
      {
        label: "Order Status",
        data: [orderStatusCounts.pending, orderStatusCounts.dispatch, orderStatusCounts.success],
        backgroundColor: ["#ff9f0d", "#f59e0b", "#10b981"],
        borderColor: "#333333",
        borderWidth: 1,
      },
    ],
    ...
    <ProtectedRoute>
      <div className="flex flex-col h-screen bg-gray-100">
        ...
        <nav className="bg-black text-white p-4 shadow-lg flex justify-between">
          ...
          <div className="sm:flex space-x-4 flex-row">
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div className="bg-white p-4 shadow-md rounded-lg">
              <h3 className="text-xl font-semibold text-[#333333]">Order Status Distribution (Bar Chart)</h3>
              <div className="h-[300px] sm:h-[400px]">
                <Bar data={barChartData} options={{ responsive: true, maintainAspectRatio: false }} />
              </div>
            </div>
            <div className="bg-white p-4 shadow-md rounded-lg">
              <h3 className="text-xl font-semibold text-[#333333]">Order Status Distribution (Pie Chart)</h3>
              <div className="h-[300px] sm:h-[400px]">
            ...
            {filteredOrders.map((order) => (
              ...
              >
                <div className="flex justify-between items-center mb-4">
                  <h3 className="font-semibold text-2xl text-black">{order.firstname} {order.lastname}</h3>
